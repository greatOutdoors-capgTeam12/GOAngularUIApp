...
export class ProductServiceModule {
  ...
  getProducts(): Observable<Product[]> {
    ...
    return this.httpClient.get<Product[]>('http://localhost:9090/Products/list').pipe(catchError(err => this.handleError));
  ...
  getProductsByCategory(prodcat: number): Observable<Product[]> {
    if (prodcat === 0) {
      ...
      return prodObs;*/
      let allProdlist = this.httpClient.get<Product[]>('http://localhost:9090/Products/list');
      return allProdlist;
  ...
  saveProduct(product: Product, pid: string): Observable<any> {
    if (pid === "0") {
     ...
     if(res === true)
     {
      ...
      return this.httpClient.post("http://localhost:9090/Products/addProduct", {
     ...
     if(res === true)
     {
      ...
      return this.httpClient.post("http://localhost:9090/Products/editProduct", {
        "prodid": product.prodid,
        "prodName": product.prodName,
        "prodBrand": product.prodBrand,
        "prodSpec": product.prodSpec,
  ...
  getProductById(): Observable<Product[]> {
    ...
    return this.httpClient.get<Product[]>('http://localhost:9090/Products/list');
  ...
  getProductsById(prodId: String): Product {
    ...
    this.httpClient.get<Product[]>('http://localhost:9090/Products/list').subscribe(prodList => this._productsList = prodList as Product[]);
  ...
  deleteProduct(prodid: string) {
    ...
    return this.httpClient.post("http://localhost:9090/Products/deleteProduct", {
      "prodid": prodid,
    })
  }
}
  ...
  addToFav(prodid : string, userid : string)
   {
    ...
    if(res === true)
    {
      return this.httpClient.post("http://localhost:9090/Wishlist/AddtoWishlist", {
      "prodid": prodid,
      "userid" : userid,
    });
  }
